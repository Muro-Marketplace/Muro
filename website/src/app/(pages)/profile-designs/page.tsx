...
export default async function ProfileDesignsPage({
  ...
  const artist =
    (params.slug && artists.find((a) => a.slug === params.slug)) ||
    artists.find((a) => a.works.length > 0) ||
    artists[0];

  if (!artist) {
    return (
      <div className="p-12 text-center">
        <p className="text-muted">No seed artists available.</p>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-10 lg:gap-14 items-start">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              <button className="px-4 py-2 rounded-full bg-accent text-white text-sm font-medium">Message</button>
              <button className="px-4 py-2 rounded-full bg-stone-900 text-white text-sm font-medium">Request placement</button>
            </div>
  ...
  return (
    <div>
      <div className="relative h-44 lg:h-56 overflow-hidden">
        <Image src={bannerSrc} alt="" fill className="object-cover" sizes="100vw" unoptimized />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10 lg:gap-14 items-start">
          <div className="min-w-0">
            <div className="relative w-28 h-28 lg:w-32 lg:h-32 rounded-full overflow-hidden bg-stone-200 border-4 border-background shadow-md mb-4">
              <Image src={artist.image || `https://picsum.photos/seed/${artist.slug}/400/400`} alt={artist.name} fill className="object-cover" sizes="128px" unoptimized />
            </div>
            <h1 className="font-serif text-3xl lg:text-4xl text-foreground leading-tight mb-1">{artist.name}</h1>
            ...
            <p className="text-foreground/85 leading-relaxed max-w-2xl text-base mb-5">{artist.shortBio}</p>
            <div className="flex flex-wrap items-center gap-3 mb-5">
              {artist.styleTags.slice(0, 5).map((tag) => (
                <span key={tag} className="text-[11px] text-muted">{formatSubStyleLabel(tag)}</span>
              ))}
              {artist.instagram && (
                <a href={`https://instagram.com/${artist.instagram.replace("@", "")}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-muted hover:text-foreground">
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" /><circle cx="12" cy="12" r="5" /><circle cx="17.5" cy="6.5" r="1" fill="currentColor" stroke="none" /></svg>
                  {artist.instagram}
                </a>
              )}
  ...
  return (
    <div>
      <div className="relative h-[520px] overflow-hidden bg-stone-900">
        ...
        <div className="absolute top-4 left-0 right-0">
          <div className="max-w-[1200px] mx-auto px-6">
            <Link href="/browse" className="text-xs text-white/80 hover:text-white">← Back to Marketplace</Link>
          </div>
        </div>
            ...
            <div className="bg-white/90 backdrop-blur-md border border-white/40 rounded-xl shadow-2xl p-6 lg:p-8 max-w-3xl">
              <div className="flex items-start gap-5">
                <div className="relative w-20 h-20 lg:w-24 lg:h-24 rounded-full overflow-hidden bg-stone-200 border-2 border-white shadow-md shrink-0">
                  <Image src={artist.image || `https://picsum.photos/seed/${artist.slug}/400/400`} alt={artist.name} fill className="object-cover" sizes="96px" unoptimized />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-[10px] text-muted uppercase tracking-[0.18em] mb-1">
                    {disciplineLabel(artist.primaryMedium, artist.discipline)}
                  </p>
                  <h1 className="font-serif text-3xl lg:text-4xl text-foreground leading-tight tracking-tight mb-1">{artist.name}</h1>
                  ...
                  <p className="text-foreground/85 leading-relaxed text-sm line-clamp-3 mb-4">{artist.shortBio}</p>
                  <div className="flex flex-wrap gap-2">
                    <button className="px-4 py-2 rounded-full bg-accent text-white text-sm font-medium">Message</button>
                    <button className="px-4 py-2 rounded-full bg-stone-900 text-white text-sm font-medium">Request placement</button>
                  </div>
    ...
    <div>
      <div className="grid grid-cols-1 lg:grid-cols-2">
        <div className="relative h-72 lg:h-[560px] bg-stone-200">
          <Image src={bannerSrc} alt="" fill className="object-cover" sizes="(max-width: 1024px) 100vw, 50vw" unoptimized />
        </div>
        ...
        <div className="p-8 lg:p-14 bg-background flex flex-col justify-center">
          ...
          <div className="flex items-center gap-4 mb-5">
            <div className="relative w-16 h-16 rounded-full overflow-hidden bg-stone-200 shrink-0">
              <Image src={artist.image || `https://picsum.photos/seed/${artist.slug}/400/400`} alt={artist.name} fill className="object-cover" sizes="64px" unoptimized />
            </div>
            <div>
          ...
          <h1 className="font-serif text-4xl lg:text-5xl text-foreground leading-[1.05] tracking-tight mb-5">{artist.name}</h1>
          ...
          <p className="text-foreground/85 leading-relaxed text-base mb-6 max-w-md">{artist.shortBio}</p>
