...
//
// Tests for signup/artist — verifies that emailRedirectTo reflects ?next=,
// falling back to /apply.

import { describe, expect, it, vi, beforeEach, afterEach } from "vitest";
import { render, fireEvent, waitFor, screen, cleanup } from "@testing-library/react";

// Use vi.hoisted so mockSignUp is available when vi.mock factories run.
...
import ArtistSignUpPage from "./page";
...
async function submitForm() {
  fireEvent.change(screen.getByPlaceholderText("Your name"), { target: { value: "Test Artist" } });
...
describe("ArtistSignUpPage — emailRedirectTo next param", () => {
  it("uses role default /apply when no ?next= is present", async () => {
    Object.defineProperty(window, "location", {
      value: { search: "", origin: "http://localhost" },
      writable: true,
    });
    render(<ArtistSignUpPage />);
    await submitForm();
    await waitFor(() => expect(mockSignUp).toHaveBeenCalled());
    const opts = mockSignUp.mock.calls[0][0].options;
    expect(opts.emailRedirectTo).toContain("next=%2Fapply");
  });

  it("forwards a safe ?next= into emailRedirectTo", async () => {
    Object.defineProperty(window, "location", {
      value: { search: "?next=%2Fmy-dashboard", origin: "http://localhost" },
      writable: true,
    });
    render(<ArtistSignUpPage />);
    await submitForm();
    await waitFor(() => expect(mockSignUp).toHaveBeenCalled());
    const opts = mockSignUp.mock.calls[0][0].options;
    expect(opts.emailRedirectTo).toContain("next=%2Fmy-dashboard");
  });

  it("falls back to /apply when ?next= is an external URL", async () => {
    Object.defineProperty(window, "location", {
