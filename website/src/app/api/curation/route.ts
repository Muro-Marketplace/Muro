...
export async function POST(request: Request) {
  ...
  if (tier.kind === "managed") {
    ...
    try {
      session = await stripe.checkout.sessions.create({
        ...
        // R2.10: keyed on the freshly inserted row id, so a transport-level
        // retry of this create cannot mint a second payable session for the
        // same request row. The row's parameters never change after insert, so
        // a repeated key can never be an idempotency-mismatch error.
  ...
  try {
    session = await stripe.checkout.sessions.create({
      ...
      // R2.10: same guard as the managed branch, keyed on the row id.
      idempotencyKey: `curation_checkout:${row.id}`,
    });
  } catch (err) {
    // D19: no Stripe session exists yet, so nothing can be paid. Removing the
