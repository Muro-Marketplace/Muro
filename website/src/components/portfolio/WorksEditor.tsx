...
export interface WorksEditorProps {
   ...
   *  heading already sits above the editor. */
  title?: React.ReactNode;
  /** Extra controls beside the works count, e.g. the portfolio page's
   *  "Edit Profile" button. */
  headerActions?: React.ReactNode;
}

/**
 * The whole works editor: the work form, the grid with drag reorder and
 * multi-select, the three bulk modals, the keyboard shortcuts and the artist's
 * shipping defaults.
 *
 * It was the body of /artist-portal/portfolio until 2026-09-06, when the
 * profile page's Works section needed the same thing. Rather than grow a second
 * implementation, the body moved here and both surfaces mount it, so a fix
 * lands in both at once.
 *
 * Works save as they go, per action, through useSaveAction. That matters where
 * this is embedded in the profile form: the profile's own Save button covers
...
export default function WorksEditor({ title, headerActions }: WorksEditorProps) {
  ...
  return (
    <>
      {/* Header. The title and any extra actions are the caller's, so the same
          row serves a full page and an embedded section. */}
