...
| **Security e2e not in CI** | `audit:e2e-security` exists but CI only runs `test:e2e` | Confirm `security-no-leaks.spec.ts` is in the default Playwright project, or add the job explicitly |
