...
| Confirm delivery: shown only when status is "shipped", explains that confirming releases payment to the artist, disables while in flight, surfaces server errors inline, and optimistically flips the local status on success. Server-side, only the buyer may set "delivered" (BUYER_STATUSES in /api/orders PATCH), matched by buyer_user_id or buyer_email, and unconfirmed orders still pay out via the 14-day cron. | Yes, correct money-boundary design and honest copy | BLOCKED. The order is at "confirmed", never reached "shipped", so the CTA does not render. |
...
| Status badges on the order: amber "Refund requested: pending review" for pending/processing, green "Refund approved", red "Refund request declined". | Yes | BLOCKED. No refund request exists on this account. |
| Post-submit confirmation: "Refund request submitted. The artist will review your request." (the API does notify the artist and platform). | Yes | BLOCKED. No refund was submitted. |
...
| Reason textarea is required; Submit stays disabled until it has non-whitespace content. | Yes | BLOCKED. Same reason. |
| Submit POSTs /api/refunds/request; on 2xx the new request is appended to local state, the form closes and resets. | Yes | BLOCKED. Same reason. |
...
| The u parameter is a raw user UUID with no signature or expiry. | FLAG: anyone who learns a user's UUID can silently flip their categories off, and link-scanning middleboxes that prefetch the GET will unsubscribe the user without a click. A signed token (HMAC of user+category) is the standard fix. | PARTLY FIXED. The unbounded-junk half is closed: the same call with u=00000000-0000-0000-0000-000000000000 returns 200 {"ok":true,"message":"You've been unsubscribed from this category."} but inserts nothing (no row exists for that id), so a foreign key now blocks non-existent users. Two residues: the API claims success for a write that did not happen, and no rate limit was observed on repeated calls. |
